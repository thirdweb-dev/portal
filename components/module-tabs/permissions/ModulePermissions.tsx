...
export const ModulePermissions: React.FC<IModulePermissions> = ({
  ...
  const onSubmit = async (permissionsData: any) => {
    ...
    (Object.entries(permissions) as [Role, string[]][]).forEach(
      ([role, members]) => {
        if (role === "transfer" && !transferIsRestricted.current) {
          return;
        }

        if (role === "lister" && !listerIsRestricted.current) {
          return;
        }

        const differentLength = data[role]?.length !== watch(role)?.length;
        const lowerCaseData = watch(role).map((address: string) =>
          address.toLowerCase(),
        );
        const notEqual =
          !differentLength &&
          (data[role] as string[]).some((address) =>
            lowerCaseData.includes(address.toLowerCase()),
          );

        if (differentLength || notEqual) {
