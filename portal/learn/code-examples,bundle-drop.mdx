...
const updateClaimConditions = async () => {
  ...
  try {
    await bundleDrop.updateClaimConditions(
      0,
...
<MdxNavigation>
  <MdxNavigationItem href="/learn/code-examples/nft-drop" title="NFT Drop" />
  <MdxNavigationItem href="/learn/code-examples/token" title="Token" next />
</MdxNavigation>
