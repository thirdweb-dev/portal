...
When creating a lazy minted batch, we recommend that you upload all token image assets to IPFS ahead of time _in an IPFS folder_.
...
const getClaimIneligibilityReasons = async () => {
  ...
  // Check if claimable.
  const claimIneligibilityReasons = await drop.getClaimIneligibilityReasons(
    quantity,
    addressToCheck,
  );
