...
Then with that signature they will be able to mint an NFT from an NFT collection that recognizes that signature.
...
Once we've created our payload and signature, we can go to the `Claimer` page to pass our payload and signature. Anyone holding both will be authorized to mint a NFT with the specific metadata we passed to the payload in the `Creator` page.
