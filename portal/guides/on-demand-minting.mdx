...
Next up we create a component folder. The first file in the components folder is for our connect wallet button. We will be using Metamask in this example.
...
```
Next create a `Layout.ts` file for our layout.
...
export default function Creator() {
  ...
  const contract = useNFTCollection("0xc335111d58913C6A382F40c8B020b4ff1ee13Ba1")
  ...
  const generateSignature = useCallback(async () => {
    ...
    const { payload, signature } = await contract.generateSignature({
...
export default function Claimer() {
  ...
  const mint = useCallback(async () => {
    const id = await contract.mintWithSignature(payload, signature as string);

    toast({
      status: "success",
      title: `Successfully minted NFT with ID: ${id}`,
    });
  }, [payload, signature, contract]);
...
[A demo of minting an NFT using an unique signature](https://portal.thirdweb.com/assets/portal/guides/minting-with-signature/demo.mov)
