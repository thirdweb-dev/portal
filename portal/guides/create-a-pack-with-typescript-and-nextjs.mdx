...
In this guide, you will learn how to work with the Pack contract and create a small app that lets users get packs of random NFTs within them. Just imagine a trading card game or baseball cards where booster packs are sold that contain a random collection of individual cards.
...
The Pack contract needs either an **NFT Collection**, a **Edition Drop**, or a **Edition Collection** to work. This is why you first need to create two contracts to get this project going. The easiest NFT contract to use with your new Pack contract is the Edition Collection, though. This is why we will go down this route in the tutorial; to give you the most straightforward experience and your new app up and running as quickly and frictionless as possible.

Go to the thirdweb dashboard and create a Edition Collection contract. Give it any name you like. In this tutorial, we will refer to it as "thirdweb Packs Edition."

Next, create a Pack contract. In this tutorial, we will refer to it as "thirdweb Packs Pack," but you can, of course, choose any name you like.
...
In this particular case, the Pack contract is owned by you. You are the only one that can create new packs, and to do this, you need access to the contract on the blockchain. When you created the contract,  we transferred the access to this contract over to your address. And as you might already know, you need a private key to sign transactions that do state changes. It would thus not be a good idea to put your private key into the source code of your frontend app that can be viewed by anyone.
...
```bash
npm install @thirdweb-dev/react@nightly @thirdweb-dev/sdk@nightly ethers dotenv
# OR
yarn add @thirdweb-dev/react@nightly @thirdweb-dev/sdk@nightly ethers dotenv
```
...
import { ThirdwebProvider } from '@thirdweb-dev';
...
The `connectWithMetamask` component contains all the logic necessary to let your users connect their wallets to your app, change their networks, and react to network changes.

### Build The Home Page

It's now time to add the Connect component to your homepage. Open `index.tsx` and remove most content from the Next.js boilerplate. We will reuse the styles of the Next.js starter and leave the outer container and the Head. Other than that, remove everything else and place your Connect component below Head. Overall, the file should now look like this:

```TypeScript:pages/index.tsx
import type { NextPage } from 'next';
import Head from 'next/head';
import styles from '../styles/Home.contract.css';
...
### Create NFTs And Fund Your contract

The Pack contract is not a standalone contract like many of the other contracts we offer. It requires an NFT contract to get its NFTs from. You can imagine a pack like a wrapper around existing NFTs. Like booster packs for trading cards only pack cards from an existing collection, and we offer three types of these contracts:

1. The **NFT Collection** contract
2. The **Edition Drop** contract
3. The **Edition Collection** contract

When you build something on your own, you could choose any of those, but as we already decided to go with a Edition collection for this tutorial, consider this choice set for now.

Go back to the thirdweb dashboard and open your "thirdweb Packs Edition" contract. Click on the button that is labeled "+ Create", and mint an NFT with an initial supply. Let's say 20 is enough for now.

![Mint](/assets/portal/guides/create-a-pack-with-typescript-and-nextjs/mint-nfts.png)

Do this at least two times so the Pack contract later has different NFTs to choose between. And when you are done with this, there is only one last thing to do before you can get back into coding: Get some LINK.

LINK is the currency of Chainlink. You need Chainlink so that your Pack contract can choose an NFT randomly. Blockchains themselves aren't very good at random numbers, which is why the contract uses an Oracle in the background. That Oracle runs on Chainlink and requires some LINK.

You can get LINK for the Mumbai Testnet [here](https://faucets.chain.link/mumbai).

When you have your LINK, go back to the thirdweb dashboard and open the "thirdweb Packs Pack." Go to **Settings** and add the LINK to the contract. This way, when a user opens a pack later, the contract can pay the required LINK for the random number.

![Funding LINK](/assets/portal/guides/create-a-pack-with-typescript-and-nextjs/funding-link.png)

When your Pack contract is funded with LINK, you have finished the last preparation step necessary. It's time to go back into the code. We will begin with the backend API that handles the on-demand creation of a pack.
...
Before you implement the handler, add a few more lines of code below your interface definitions. The contract address of the pack contract can be a constant, you will need it multiple times. After that follows the setup of the thirdweb SDK. Instantiating it each time a user makes a request makes no sense, so it can stay outside of the handler.

```TypeScript:pages/api/pack.ts
const PACK_CONTRACT_ADDRESS = process.env.PACK_CONTRACT_ADDRESS as string;
...
const contract = SDK.getPack(PACK_CONTRACT_ADDRESS);
```
...
In the next step, we will add the logic to create a pack of NFTs, and the contract gives you the opportunity to add a logo to it. In our case, we do of course use our own logo, located at `public/thirdweb_logo.png`, but you can add any other image you like. If you currently don't have a suitable image, don't worry, you have permission to use our logo here, of course. Just download it and place it inside your project.
...
try {
  const createResult = await contract.create(pack);
  const { id } = createResult;
  
  // transfer the pack over to the user so they can open it themselves
  await contract.transfer(requestData.address, id, BigNumber.from(1));

  res.status(200).json({ 
    packId: id,
    packAddress: PACK_CONTRACT_ADDRESS
  });
} catch (error: any) {
  res.status(500).end();
}
...
After reading the request data, the code creates an option object for the pack contract. The property `assetsContract` contains the contract address of a Edition or drop contract. In this case, it is the address of the contract "thirdweb Packs Edition".
...
`assets` is the most important property of the options object. Here you define all NFTs from the `assetsContract` that can potentially land inside a pack. Add in at least the ids of the NFTs you created within your Edition contract.

The API creates that pack on demand. And when that is successful, it transfers the pack over to the address the caller sent with their request. After that, the id of the pack and the contract address of the pack contract are returned to the user.
...
Additionally add the following to `styles/Home.contract.css`:

```CSS:styles/Home.contract.css
.button {
