...
This guide gives you a simple React **component** to create an `auction` and a **component** for a `connect wallet` button. Check out our repo with examples at Github [here]().
...
```bash
npm install @thirdweb-dev/sdk @thirdweb-dev/react @thirdweb-dev/hooks ethers
```
...
=======
    <ThirdwebProvider desiredChainId={supportedChainIds}>
      <Connect />
      <AuctionComponent /> /*render the auction button*/
>>>>>>> Stashed changes
    </ThirdwebProvider>
  );
}
...
Next up we instantiate the sdk, marketplace contract and create variables with our contracts.
...
// We won't always have a provider, and so we need to be sure to
  ...
  const market = useMemo(
    () =>
      sdk.getMarketplace("0xCb67A96FAd36D8c24f192B9eDaD5fF3c7A7A867f"),
    [sdk]
  );
...
const AuctionComponent = () => {
  ...
  const market = useMemo(
    () =>
      sdk.getMarketplacecontract("0xCb67A96FAd36D8c24f192B9eDaD5fF3c7A7A867f"),
    [sdk]
  );
