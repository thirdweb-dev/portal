...
2. Select the chain you want to create the contract on: elect the network you want to deploy to: Mainnet, Polygon, Fantom, Avalanche, Testnet.
...
import { useSigner, useMetamask, useNFTCollection, useAccount } from "@thirdweb-dev/react";
...
const RenderAllNFTComponent = () => {
  ...
  useEffect(() => {
    ...
    const nftContractAddress = "0x2a7e8270e8E75d97A34443054a1B7B5A6EDe4a34"
    const nft = useNFTCollection(nftContractAddress)
...
const MintButton = () => {
  ...
  return (
    {signer? <button onClick={onMintHandler}>Mint Sword NFT</button> : <ConnectWallet>}
    )
};
```
...
[**Check the code on GitHub**](https://github.com/thirdweb-dev/mint-nft-nextjs-example).
