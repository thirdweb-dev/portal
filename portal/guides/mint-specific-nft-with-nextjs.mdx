...
The easiest way to set up a Next.js app is to use the template which can be [found here.](https://github.com/thirdweb-example/next-typescript-starter) You can “Use this template” and create your own repo, which can be cloned and opened in your code editor. 
...
Don’t forget to add the `ChakraProvider`, afterwhich your `_app.tsx` should look like this:
...
**NB:** If you open `_app.tsx` (in your `pages` folder), you’ll see that you can set the chain on which you want your app to work. In this tutorial, we are working on the Rinkeby testnet so we have changed it from this: 
...
We will display each of our NFTs using an `nftCard` component.  Chakra’s `Flex` , `Box` and `Image` components will help us build each NFT card. You can create a new file called `nftCard.tsx` in a new folder called `components` and add the following code:
...
const NftCard = (props: any) => {
  return (
    <Box maxW='sm' borderWidth='1px' borderRadius='lg' overflow='hidden'>
      <Image src={props.nftDetails.url} alt='NFT image' />
      ...
      <Flex p='1rem' alignItems='center' flexDir='column'>
        ...
        <Button colorScheme='purple' m='0.5rem'>Mint</Button>
      </Flex>
    </Box>
  );
};
...
### Create NFT Metadata and Display NFTs

In `index.tsx`, we can add the NFT metadata with a `useState` so that we can work with it later. We include the `name`, `description`, `url`, and `price` for each NFT and these attributes will be used to generate the signatures and mint the NFTs. 

Once the code to display the NFTs has been added, `index.tsx` should look like this:

```tsx
import { Flex, Heading, SimpleGrid } from '@chakra-ui/react';
import { useAddres, useMetamask } from '@thirdweb-dev/react';
import type { NextPage } from 'next';
      ...
      {address ? (
        <Flex mt='5rem' alignItems='center' flexDir='column'>
          ...
          <SimpleGrid justifyItems='center' columns={3} spacing={10}>
            {nftMetadata.map((nft) => (
              <NftCard key={nftMetadata.indexOf(nft)} nftDetails={nft} />
            ))}
          </SimpleGrid>
        </Flex>
      ) : (
...
We will connect using the `useNFTCollection` hook in `index.tsx`. Add the hook to your import statement from `@thirdweb-dev/react` and then use it with the contract address which can be found in your dashboard. 
...
The signature generation has to be done by the wallet that owns the NFT Collection contract. To do this, we use server-side code to connect to the SDK and the contract and then generate the NFT. We will use Nextjs’ built-in API capabilities by creating an `api` folder inside the `pages` folder. Inside `api`, create a new file called `signatureGeneration.ts`. 
...
Now, we can mint the NFT using the `payload` and `signature` from the signature generation. We will implement this in `index.tsx` and the code is below. 

You’ll notice 3 other things in the code...

1. We pass a `loading` param to the NFT card so that we dynamically show when we are loading and when the user can mint an NFT. 
2. Once an NFT has been minted, we change the state to mark that NFT and minted which will then replace the mint button in the `NftCard` with a message saying that the NFT can’t be minted. 
3. We pass the minting function into each `NftCard` component

The minting function is implemented in `index.tsx` but there are also changes in `nftCard.tsx`. Please follow along in the comments. 
  ...
  async function onClick(
    ...
    const response: any = await fetch('/api/generateSignature', {
      ...
      body: JSON.stringify({
		...
		// If there is a response from the API, mint the NFT
    if (response) {
...
Congrats 🎉 You’ve built an NFT collection that allows users to select which 1-of-1 NFT they want to mint☺️ If you want to explore what else you can do with thirdweb, please check out the [docs](https://docs.thirdweb.com/) and [guides](https://portal.thirdweb.com/guides) here.