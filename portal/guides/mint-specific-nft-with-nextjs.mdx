...
We will display each of our NFTs using an `nftCard` component.  Chakra’s `SimpleGrid`, `Flex` , `Box` and `Image` components will help us build each NFT card. You can create a new file called `nftCard.tsx` in a new folder called `components` .
