...
In you metamask wallet, click on the three dots, then click account details. You will see an option to export private key there. Export your private key and paste it in the `PRIVATE_KEY` variable.
...
From the frontend we will get the `tokenName`, `tokenId`, and the `address` of the person we are going to mint the NFT to. I am only getting these parameters to make sure that people can't manipulate the data. So, get these parameteres from the response body-
...
In this variable we are getting the token from the `nft.tokens` array. If someone passes a token that is not in the list we will use the `MATIC` token.
