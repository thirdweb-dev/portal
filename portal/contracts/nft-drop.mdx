...
draft: false
summary: "The NFT Drop contract gives you a distribution mechanism for your NFTs."
date: "2022-02-13"
updated: "2022-03-14"
---
...
![Dashboard option](/assets/portal/contracts/nft-drop/dashboard.png)

thirdweb's `NFT Drop` contract lazy mints an NFT and makes it available to be claimed by your users. Lazy minting is when the creator defines conditions for a third party (your audience) to come in and mint the NFT. You choose what the underlying token is you want to distribute. So it can be a 1 of 1 NFT or 1 of many. Our `NFT Drop` contract gives you the mechanism to distribute that token.

![Contract options](/assets/portal/contracts/nft-drop/options.png)

Click [here](/contracts) to learn more about contracts and how to create them.

## How it works

Make sure you choose the right contract. If you want your **drop** to include 1 of 1 NFTs, choose the `NFT Drop` contract. If you want to mint 1 of many NFTs, choose the `Edition Drop` contract.
...
The following use cases are suited for the `NFT DROP` contract and are driven

1. Setup early access NFT (click here for a guide on that)
2. List an NFT for sale on a Marketplace (you would also need the `Marketplace` contract for this)
...
![Embed settings](/assets/portal/contracts/nft-drop/embed-example.png)

![Embed](/assets/portal/contracts/nft-drop/embed.png)
...
![delayed reveal dashboard](/assets/portal/contracts/nft-drop/delayed.png)
...
To install the necessary packages for TypeScript or Python on your (local) machine and instantiate the SDK in your code, check out [this](https://portal.thirdweb.com/contracts/connect-to-blockchain) guide to help you with that. Note, you can instantiate the SDK in multiple ways, so make sure to check out that guide.

You can find the contract address in the thirdweb dashboard, inside your project or inside the `NFT Drop` contract itself.

![contract addresses](/assets/portal/contracts/nft-drop/contract-addresses.png)
...
2. To check out guides for this contract click [here](portal/guides/nft-drop).
...
<MdxNavigation>
  <MdxNavigationItem
    ...
    title="Edition"
  />
  <MdxNavigationItem href="/contracts/token" title="Token" next />
</MdxNavigation>
