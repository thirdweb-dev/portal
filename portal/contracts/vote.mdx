...
1. `Governance Token Address` is an ERC-20 contract (our **Token** contract) to work with the **Vote** contract.
2. `Proposal Token Threshold` is the amount of tokens a wallet should hold to make a proposal
3. `Voting Delay` is the time period after which members can vote on the proposal
4. `Voting Period` is the period how long members can vote. This needs to be specified in numbers of blocks. Check your chain how many block are generated in an hour or day, so you know what number to fill in here.
5. `Voting Quorum` determines what the minimum percentage of votes are needed for the proposal to pass.
