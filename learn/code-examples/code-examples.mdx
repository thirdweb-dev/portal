...
const getBalance = async () => {
  let bal = await drop.balance();

  console.log(bal.toNumber());  // Since bal is a BigNumber object.
};
```
...
const getBalance = async () => {
  let bal = await drop.balanceOf("<TARGET_ADDRESS>");
...
const burnToken = async() => {
    ...
    let tokenIdBigNumber = new ethers.BigNumber.from(tokenId);

    await drop.burn(tokenIdBigNumber)
    .then((data) => console.log(data))
    .catch((err) => console.log(err));
}
```
...
const canClaim = async () => {
    let quantity = 1;
    let addressToCheck = "<MODULE_ADDRESS>";

    // Convert quantity to big number.
    let quantityBigNumber = new ethers.BigNumber.from(quantity);

    // Check if claimable.
    const isClaimable = await drop.canClaim(quantityBigNumber, addressToCheck)
...
const claim = async () => {
  let quantity = 1;
  let receiverAddress = "<TARGET_ADDRESS>";

  let quantityBigNumber = new ethers.BigNumber.from(quantity);

  await drop
    .claimTo(quantityBigNumber, receiverAddress)
    .then((data) => console.log(data))
    .catch((err) => console.log(err));
};
```
...
const getClaimIneligibilityReasons = async () => {
  let quantity = 1;
  let addressToCheck = "<TARGET_ADDRESS>";

  // Convert quantity to big number.
  let quantityBigNumber = new ethers.BigNumber.from(quantity);

  // Check if claimable.
  const claimIneligibilityReasons = await drop.getClaimIneligibilityReasons(quantityBigNumber, addressToCheck);
